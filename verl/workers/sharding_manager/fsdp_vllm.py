...
class FSDPVLLMShardingManager(BaseShardingManager):
    ...
    def _record_cleanup_failure(self, original_error, cleanup_error, action):
        ...
        try:
            original_error.fsdp_vllm_cleanup_error = cleanup_error
        except BaseException:
            pass
    ...
    def _rng_states_equal(current_state, expected_state):
        if isinstance(current_state, torch.Tensor) and isinstance(expected_state, torch.Tensor):
        ...
        def run_cleanup(action, cleanup_fn):
            ...
            try:
                ...
                if cleanup_error is None:
                    cleanup_error = error
                self._record_cleanup_failure(cleanup_error, error, action)
                return False
            return True
