...
_MODULE_PATH = _REPO_ROOT / "verl/workers/rollout/vllm_config.py"
_SPEC = importlib.util.spec_from_file_location("vllm_config", _MODULE_PATH)
assert _SPEC is not None and _SPEC.loader is not None
_MODULE = importlib.util.module_from_spec(_SPEC)
_SPEC.loader.exec_module(_MODULE)

build_vllm_sampling_params_kwargs = _MODULE.build_vllm_sampling_params_kwargs
resolve_vllm_engine_seed = _MODULE.resolve_vllm_engine_seed
...
def test_all_vllm_rollout_paths_use_shared_sampling_config_builder():
    rollout_dir = _REPO_ROOT / "verl/workers/rollout/vllm_rollout"
    paths = [
