...
"""Canonical ALFWorld and WebShop task splits used during training."""
...
def _download_fairness_file(
    ...
    repo_id = _DATASETS[environment]
    errors: list[str] = []
    destination.parent.mkdir(parents=True, exist_ok=True)

    for endpoint in _candidate_endpoints():
        url = (
            f"{endpoint}/datasets/{repo_id}/resolve/main/"
            f"fairness/{filename}"
        )
        try:
...
def fairness_file(environment: str, split: str) -> Path:
    """Return a local fairness JSONL path, downloading it on first use."""
...
def load_fairness_rows(
    ...
    """Load a canonical fairness split and validate its stable identities."""
    ...
    if environment == "alfworld":
        identities = [
            row.get("metadata", {}).get("gamefile")
            for row in rows
        ]
        identity_name = "metadata.gamefile"
    else:
        identities = [
            row.get("metadata", {}).get("goal_idx")
            for row in rows
        ]
        identity_name = "metadata.goal_idx"
...
def alfworld_gamefiles(split: str) -> list[str]:
    """Load ALFWorld game paths in their canonical fairness order."""
...
def alfworld_fairness_split(
    *,
    ...
    """Choose the training or requested ALFWorld evaluation split."""
...
def webshop_goal_indices(split: str) -> list[int]:
    """Load WebShop goal indices in their canonical fairness order."""
...
def webshop_goal_fingerprint(goals: list[dict[str, Any]]) -> str:
    """Hash resolved WebShop goals to compare worker-local task identity."""
...
def canonical_validation_splits(environment: str) -> tuple[str, ...]:
    """Return the exhaustive canonical validation splits."""
...
def canonical_validation_chunks(
    ...
    *,
    ...
    """Partition one exhaustive fairness split without changing its order."""
    ...
    values: list[Any]
    if environment == "alfworld":
        values = alfworld_gamefiles(split)
    else:
        values = webshop_goal_indices(split)
    return tuple(
...
def alfworld_worker_gamefiles(
    *,
    ...
    """Assign each evaluation worker one fixed game in manifest order."""
...
def webshop_reset_goal_indices(
    *,
    ...
    """Choose training goals or return the fixed validation order."""
